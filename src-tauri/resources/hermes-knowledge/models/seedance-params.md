...
- 文件名：`@hero.png`（含扩展名）
- **连线强制匹配**（无扩展名也可）：`@背景音乐` 匹配 `背景音乐.mp3`；源节点**标签**（如「主角立绘」）；分镜**镜号**（如 `S03`）
- 解析实现：`src/lib/seedance/promptBuilder.ts` + `videoPromptAtTokens.ts`（生成时合并上游标签）
- Hermes `film.shot_to_video_prompt` 默认写纯文本即可；批量出视频前会同步上游图到 `reference*Paths`。若在 prompt 中写 `@图1`，须已连线且顺序一致
