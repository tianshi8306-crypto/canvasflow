...
export const IMAGE_MODEL_MENU_Z = 1200;

const MENU_SELECTOR = ".igp-model-menu--portal";
...
export function ImageModelPicker({ models, value, onChange, onOpenChange, loading = false }: Props) {
  ...
  const menu =
    open && selectableModels.length > 0 && menuStyle && typeof document !== "undefined"
      ? createPortal(
          <div
            className="igp-model-menu igp-model-menu--portal imageGenPanel--minimal"
            style={menuStyle}
            role="listbox"
            aria-label="图片模型"
                ...
                return (
                  ...
                  >
                    ...
                    <span className="igp-model-item-body">
                      ...
                      {m.subtitle ? (
                        <span className="igp-model-item-subtitle">{m.subtitle}</span>
                      ) : null}
                    </span>
                    <span className="igp-model-item-estimate">{m.estimateLabel}</span>
                  </button>
                );
              })}
            </div>
